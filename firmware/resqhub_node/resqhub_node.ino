 ...
 *
 * ──────────────── ПІДКЛЮЧЕННЯ ────────────────
 *  ACS712  VCC → 5V (VIN)      GND → GND      OUT → GPIO34   ← ADC1! (важливо)
...
// ─────────────── ПІНИ ───────────────
const int PIN_ACS712 = 34;   // ADC1 (тільки-вхід) — сигнал ACS712 OUT
...
// ─────────────── ACS712 ───────────────
// Чутливість (В на Ампер): 5A=0.185, 20A=0.100, 30A=0.066. Постав свою.
const float ACS712_V_PER_A = 0.100f;
